...
abstract class AbstractTestUpdateOperation extends AbstractTestResourceIntegration
{
    ...
    public function testUpdateFlow(): void
    {
        ...
        $response = $this->executeUpdateOperation();

        $this->assertCount(2, $this->requestHistory);
        $this->assertAuthRequestValid();
        $this->assertUpdateRequestValid();
     ...
     * @throws \Apiera\Sdk\Exception\Mapping\MappingException
     */
    abstract protected function executeUpdateOperation(): ResponseInterface;
    ...
    protected function assertUpdateRequestValid(): void
    {
        $request = $this->requestHistory[1]['request'];
        $this->assertEquals('PATCH', $request->getMethod());
        $this->assertEquals('Bearer test_token', $request->getHeader('Authorization')[0]);
        $this->assertEquals('application/merge-patch+json', $request->getHeader('Content-Type')[0]);
        $this->assertEquals(
